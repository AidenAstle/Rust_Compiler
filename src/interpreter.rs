...
impl Runtime {
  ...
  pub fn run(&mut self, node: &Node) -> Result<Value, &'static str> {
    ...
    match node {
        ...
        Node::Program { children } => {
            for n in children {
                match n {
                    ...
                    // CUT 1 : Conditionals
                    // If the child node is a `Conditional Expression`, add it as the body of a new `main` function.
        ...
        // INTERPRETER CODE FOR CUT 1 : Conditionals
        Node::ConditionalExpression { name, children } => {
            /*
                Checks that both sides of the conditional are of comparable types i.e.
                both are numbers or booleans
            */
            match (self.run(&children[0]), self.run(&children[1])) {
                (Ok(Value::Number(lhs)), Ok(Value::Number(rhs))) => {
                    match name.as_ref() {
                        ...
                        "==" => {Ok(Value::Bool(lhs == rhs))},
                        "!=" => {Ok(Value::Bool(lhs != rhs))},
                        _ => Err("Invalid operation for type : number")
                    }
                },
                (Ok(Value::Bool(lhs)), Ok(Value::Bool(rhs))) => {
                    match name.as_ref() {
                        "==" => {Ok(Value::Bool(lhs == rhs))},
                        "!=" => {Ok(Value::Bool(lhs != rhs))},
                        _ => Err("Invalid operation for type : boolean")
                    }
                },
                _ => Err("Type Mismatch"),
            }
        ...
        Node::IfStatement { children } => {
            let mut same_type = true;   //match and for loop check that all return types match the first
            match self.run(&children[0]){
                Ok(Value::Bool(..)) => {
                    for n in children {
                        match self.run(n) {
                            Ok(Value::Bool(..)) => {},
            ...
            if (same_type) { //if all types match
                for n in children{
                    match self.run(n){
                        Ok(..) => {
                            return self.run(n);
                ...
                return Err("No return value found"); //should always return a val because there must be an else statement
            } else {
        ...
        Node::IfCase { children } => {
            ...
            match condition {
                Ok(Value::Bool(true)) => {
                    ...
                    match children.last() {
                        Some(..) => {
                            self.run(children.last().expect("")) // the last child should always be the return
                        },
                        None => {Err("No Return Statement")}
                    }
                },
                _ => {Err("Next")} //allows the above for loop to skip this If node if the condition is false
            }
        },
        Node::ElseCase { children } => {
            for n in children {
                self.run(n);
            }
            match children.last() {
                Some(..) => {
                    self.run(children.last().expect(""))
                },
                None => {Err("No Return Statement")}
            }
        },
